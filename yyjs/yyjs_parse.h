...
#define YY_SCOPE_PRE_JUMP   8
    uint16_t flags, n_iter_i;
    uint32_t iter_i[10];
};

SLIST_HEAD(yy_parse_scope_q, yy_parse_scope);
...
#define YY_CONTEXT_ARROW    4
    ...
    struct yy_parse_scope_q scope_chain;
    int local_vars[80];
...
#define YY_PARSER_BASE_1    4 /* Targeting stock Lua VM */
...
void yy_parser_push_context(struct yy_parse_state *, int arrow);
int yy_parser_pop_context(struct yy_parse_state *);
struct yy_parse_scope *yy_parser_push_scope(struct yy_parse_state *);
...
enum ExprType { T_LV_REF = 1, T_PRI, T_PROTO, T_STR, T_INT };
...
extern long yyjs_dump_bc(struct yy_parse_state *, struct yy_dump_state *);
