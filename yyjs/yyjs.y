...
#define ACTION(type) yy_bcg_do_action(state, ACTION_##type)
#define OPERATOR(type, x) yyval = yy_emit_operator(state, type, x)
#define LOCAL_DECLARE(id) if (yy_parser_alloc_local(state, id) < 0) YYERROR;
...
_TOpen: { $$ = ACTION(OPEN_TABLE); };
_M: { OPERATOR(0, $0); };
_B: { OPERATOR(P_TOBIT, $0); };
...
ElementList:
    _ExpressionOrObject { yy_emit_array_field(state, 0, 0), $$ = 1; } |
    ',' _ExpressionOrObject { yy_emit_array_field(state, 1, 0), $$ = 2; } |
    ElementList ',' _ExpressionOrObject
        { yy_emit_array_field(state, $1, 0), $$ = $1 + 1; }
    ;
...
UnaryExpression:
    ...
    '+' UnaryExpression { OPERATOR(P_TOBIT, $2); } |
    '-' UnaryExpression {
        if (expr_type($2) == T_INT)
            $$ = expr_tag(T_INT) | (uint16_t)-$2;
        else if (expr_type($2) > T_INT) $$ = $2 ^ (1ull << 63);
        else OPERATOR(P_UNM, $2);
    } |
    '!' UnaryExpression {
...
ExponentiationExpression:
    UnaryExpression |
    UnaryExpression _B P_POW ExponentiationExpression { OPERATOR(P_POW, $4); }
    ;
...
MultiplicativeExpression:
    ExponentiationExpression |
    MultiplicativeExpression _B '*'
        ExponentiationExpression { OPERATOR('*', $4); } |
    MultiplicativeExpression _B '/'
        ExponentiationExpression { OPERATOR('/', $4); } |
    MultiplicativeExpression _B '%'
        ExponentiationExpression { OPERATOR('%', $4); }
...
AdditiveExpression:
    MultiplicativeExpression |
    AdditiveExpression _B '+'
        MultiplicativeExpression { OPERATOR('+', $4); } |
    AdditiveExpression _B '-'
        MultiplicativeExpression { OPERATOR('-', $4); }
    ;
...
ShiftExpression:
    AdditiveExpression |
    ShiftExpression _B P_SHL AdditiveExpression { OPERATOR(P_SHL, $4); } |
    ShiftExpression _B P_SHR AdditiveExpression { OPERATOR(P_SHR, $4); } |
    ShiftExpression _B P_ASHR AdditiveExpression { OPERATOR(P_ASHR, $4); }
    ;
...
BitwiseANDExpression:
    EqualityExpression |
    BitwiseANDExpression _B '&' EqualityExpression { OPERATOR('&', $4); }
    ;

BitwiseXORExpression:
    BitwiseANDExpression |
    BitwiseXORExpression _B '^' BitwiseANDExpression { OPERATOR('^', $4); }
    ;

BitwiseORExpression:
    BitwiseXORExpression |
    BitwiseORExpression _B '|' BitwiseXORExpression { OPERATOR('|', $4); }
    ;

/* ECMAScript 2015 12.12 Binary Logical Operators */

_And: P_AND { OPERATOR(P_AND, $0); };
_Or: P_OR { OPERATOR(P_OR, $0); };
...
ConditionalExpression:
    LogicalORExpression |
    LogicalORExpression '?' { OPERATOR(P_AND, $1); }
    _ExpressionOrObject ':' {
        $$ = yy_emit_jmp(state, 0);
...
LexicalBinding:
    ...
    BindingIdentifier { yy_set_identifier_tag(state, $1); }
        Initializer { LOCAL_DECLARE($1); } |
    '{' BindingPropertyList '}' Initializer { OPERATOR('}', $2); }
    ;
...
TryStatement:
    _TryBody Catch { ACTION(EMIT_CHECK_RET); } |
    _TryBody Catch Finally { ACTION(EMIT_CHECK_RET); } |
    _TryBody Finally { ACTION(EMIT_RETHROW); }
    ;

Catch:
    KEYWORD_CATCH '(' BindingIdentifier ')' {
        yy_bcg_open_scope(state, KEYWORD_CATCH);
        yy_parser_alloc_local(state, $3);  /* Catch error object */
    } Block {
        yy_bcg_close_scope(state, 0);
    } |
...
FunctionRestParameter:
    P_ELISION BindingIdentifier {
        ACTION(CAPTURE_VARARG);
        LOCAL_DECLARE($2);
    } |
    ;
...
FormalParameter:
    BindingIdentifier {
        ACTION(PREPARE_OPTIONAL), $$ = yy_emit_jmp(state, 0);
    } Initializer {
        yy_fill_jmp_placeholder(state, $2);
        LOCAL_DECLARE($1 | YY_ID_HINT_ARG);
    } |
    BindingIdentifier { LOCAL_DECLARE($1 | YY_ID_HINT_ARG); }
    ;
...
ArrowFunction:
    ArrowParameters P_ARROW {
        ...
        if ($1 != -1) LOCAL_DECLARE($1 | YY_ID_HINT_ARG);
    } ConciseBody {
        OPERATOR(0, yy_close_function(state));
    }
    ;
...
ClassDeclaration:
    KEYWORD_CLASS BindingIdentifier ClassTail {
        yy_emit_assignment(state, GLOBAL($2), 0);
    }
    ;
...
ClassTail:
    { ACTION(OPEN_CLASS); } ClassHeritage '{' ClassBody '}'
...
ClassElement:
    ...
    KEYWORD_STATIC IdentifierName Initializer ';'
...
AsyncArrowFunction:
    KEYWORD_ASYNC ArrowParameters P_ARROW {
        yy_bcg_open_scope(state, P_ARROW);
        if ($2 != -1) LOCAL_DECLARE($2 | YY_ID_HINT_ARG);
    } ConciseBody {
        OPERATOR(KEYWORD_ASYNC, yy_close_function(state));
    }
    ;
