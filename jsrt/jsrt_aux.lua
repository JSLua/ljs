...
if jit then
    -- Implement TypedArray using luajit ffi
    local ffi = require "ffi"
    local function defineTypedArray(ctype)
        return function(_, length)
            assert(type(length) == "number", "length expected")
            return ffi.new(ctype .. "[" .. length .. "]")
        end
    end

    Uint8Array = defineTypedArray("unsigned char")
    Uint16Array = defineTypedArray("unsigned short")
    Uint32Array = defineTypedArray("unsigned int")
    Int8Array = defineTypedArray("signed char")
    Int16Array = defineTypedArray("signed short")
    Int32Array = defineTypedArray("signed int")
    Float32Array = defineTypedArray("float")
    Float64Array = defineTypedArray("double")
end
...
if Promise ~= nil then
    function Promise.prototype:catch(fun)  -- Use then() internally
        return self["then"](self, nil, fun)
    end
