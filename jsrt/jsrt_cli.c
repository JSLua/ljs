...
static void interactive()
{
    ...
    while (1) {
        ...
        if (!line) break;  /* EOF */
            ...
            if (!buffer) buffer = line, length = l_siz;
            else if ((buffer = realloc(buffer, length + l_siz))) {
                buffer[length - 1] = '\n';
                memcpy(buffer + length, line, l_siz), length += l_siz;
                free(line);
            }
            else return (void)luaL_error(L, "insufficient memory");
        } else continue;

        /* Try loading user input */
        lua_settop(L, 1);
        if (luaJS_loadstring(L, buffer, length, "@stdin") == -1) {
            const char *err = lua_tostring(L, -1);
            if (strstr(err, "syntax error, unexpected end of file"))
                continue;  /* More lines to be read */
            printf("%s\n", err);
            goto next;
        }
...
int main(int argc, char *argv[]) {
    ...
    if (luaJS_loadfile(L, infile, *out_file && strip) != 0) {
