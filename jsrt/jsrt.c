...
static int gen_next(lua_State *L)
{
    ...
    if (th == L) return luaL_error(L, "Generator is already running");
        ...
        if (arg && lua_isboolean(L, 2))
            return luaL_error(L, "could not pass boolean via next()");
        if (arg)
            lua_xmove(L, th, 1);
...
static int gen_iter_next(lua_State *L)
{
    luaL_checktype(L, 1, LUA_TTHREAD);
    lua_State *th = lua_tothread(L, 1);
    if (th == L) return luaL_error(L, "Generator is already running");
    ...
    switch (status) {
        case LUA_OK:
            ...
            break;
        case LUA_YIELD: status = lua_resume(th, 0); break;
        default: return 0;
    }
    switch (status) {
        case LUA_YIELD:
...
extern int promise_new_async(lua_State *L);

static int lib_async(lua_State *L)
{
    luaL_checktype(L, 1, LUA_TFUNCTION);
    lua_pushvalue(L, lua_upvalueindex(1));  /* Resolver */
    lua_pushvalue(L, 1);  /* Async function body */
    lua_call(L, 1, 1);
    lua_pushcclosure(L, promise_new_async, 1);
    return 1;
}

static int lib_await(lua_State *L)
{
    luaL_checkany(L, 1), lua_settop(L, 1);
    if (luaL_testudata(L, 1, "JS:Promise"))
        return lua_yield(L, 1);  /* Let the awaiter handle this */
    return 1;
}
...
/* Helper implemented in Lua to guard async functions */
const char resolver[] =
        "local body = ...\n"
        "return function(resolve, reject, ...)\n"
        "    local ok, ret = pcall(body, ...)\n"
        "    if not ok then resolve = reject end\n"
...
int luaopen_jsrt(lua_State *L)
{
    ...
    /* jsrt.async */
    luaL_loadbuffer(L, resolver, sizeof(resolver) - 1, "async helper");
    ...
    if (!js_path)
        js_path = "./?.js;./?.jbc;/usr/local/share/ljs/?.js;"
                  "/usr/share/ljs/?.js;/usr/share/ljs/?.jbc";
